...
public class ValueAnimator extends Animator {
    ...
    public void setValues(PropertyValuesHolder... values) {
        ...
        mValues = values;
        // 一个细节，初始话mValuesMap的时候只使用了values的长度
        mValuesMap = new HashMap<String, PropertyValuesHolder>(numValues);
        // 将 PropertyValuesHolder遍历添加到 mValuesMap
    ...
    public ValueAnimator setDuration(long duration) {
        ...
        // 更新动画的比例时间，获取真正的动画持续时间
        updateScaledDuration();
        return this;
    }

    /**
     * 这个方法用法用于根据没有修改前的动画时间乘上动画时间的比例尺度，然后确定最终的动画持续时间
     */
    ...
    protected static class AnimationHandler {
        ...
        void doAnimationFrame(long frameTime) {
            ...
            while (mPendingAnimations.size() > 0) {
                // 将待执行的动画都克隆到另一个列表中，然后删除mPendingAnimations的元素
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    if (anim.mStartDelay == 0) {
                        // this：这个是当前的AnimationHandler
                        // 这个方法会设置了估值器，将动画对象添加到可以运行的动画列表中
            ...
            // 现在开始进行执行所有可以运行的动画，通过animationFrame()方法的返回值确定动画是否end
            int numAnims = mAnimations.size();
            // 遍历所有的动画对象，将这些对象都添加到 mTmpAnimations，防止这些动画对象被修改了
        ...
        /**
         * 调度动画执行，将动画交给Choreographer执行
         */
        private void scheduleAnimation() {
            if (!mAnimationScheduled) {
                // mAnimate是一个 Runnable对象，在这里把动画的执行交个了mChoreographer开启
        ...
        // 这个对象会被Choreographer使用，并执行
        final Runnable mAnimate = new Runnable() {
            @Override
            public void run() {
                // 一旦动画被执行就重置动画执行状态
                mAnimationScheduled = false;
                // mChoreographer.getFrameTime()获取上一帧的执行时间
                // 执行动画帧
    ...
    /**
     * 由这个方法知道，一个属性动画的执行会回调所有的动画监听器，虽然是这样，但是由于这个
     * mListeners不是static，所以还是不会对其他属性动画对象有所影响，它只会回调当前属性动画
     * 对象的 onAnimationStart() 方法
     */
    private void notifyStartListeners() {
        // 由 mStartListenersCalled标志位可以知道，onAnimationStart() 方法只会调用一次
     ...
     *
     * @param playBackwards 如果为true，那么就表明这个动画执行是要重复执行的
     */
    private void start(boolean playBackwards) {
        ...
        // 这里之所以会再次计算动画时间，可能是要保证动画的最终时间
        updateScaledDuration();

        // 获取当前线程的 AnimationHandler对象，如果不存在就创建
        AnimationHandler animationHandler = getOrCreateAnimationHandler();

        // 将当前的动画对象添加到待运行动画的列表中
        animationHandler.mPendingAnimations.add(this);

        // 如果动画没有开始延迟时间，那么就设置一些信息
        // 比如：回调动画监听器告诉使用者，动画要开始运行了
        if (mStartDelay == 0) {
            ...
            // 标志动画正在运行
            mRunning = true;
            // 如果这个动画没有延迟开始时间，那么调用 AnimatorListener中的
            // onAnimationStart() 方法告诉使用者动画开始运行了
    ...
    private void startAnimation(AnimationHandler handler) {
        ...
        // 初始化动画的一些配置，如：TypeEvaluator估值器
        initAnimation();
        // 添加到正在可以运行的动画列表中
        handler.mAnimations.add(this);
        if (mStartDelay > 0 && mListeners != null) {
            // 如果该动画对象有延迟开始事件，那么在 start() 方法中就不会调用 onAnimationStart()
            // 方法，所以这里需要再次通知用户
            notifyStartListeners();
        }
    }
