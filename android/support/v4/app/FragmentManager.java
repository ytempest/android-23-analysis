        ...
        if (f.mState < newState) {
            ...
            switch (f.mState) {
                case Fragment.INITIALIZING:
                    ...
                    if (!f.mRetaining) {
                        // 这里会回调Fragment的onCreate方法
                        f.performCreate(f.mSavedFragmentState);
                    }
                    f.mRetaining = false;
                    // 这里是指如果Fragment是声明在Layout中的话，会走这里
                    if (f.mFromLayout) {
                ...
                case Fragment.CREATED:
                    if (newState > Fragment.CREATED) {
                        ...
                        if (!f.mFromLayout) {
                            ...
                            if (f.mView != null) {
                                ...
                                if (container != null) {
                                    ...
                                    // 这里会将Fragment创建的View添加到指定的父布局中
                                    container.addView(f.mView);
                                }
                                if (f.mHidden) f.mView.setVisibility(View.GONE);
                                // 这里会回调Fragment的onViewCreated方法，这时Fragment的View是已经添加到布局中的
                                f.onViewCreated(f.mView, f.mSavedFragmentState);
                            } else {
                                f.mInnerView = null;
                            }
                        ...
                        // 这里会回调Fragment的onActivityCreated方法
