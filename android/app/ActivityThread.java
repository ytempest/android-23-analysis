...
public final class ActivityThread {
    ...
    /**
     * Descirption：这个类实现了 IApplicationThread AIDL的接口方法
     */
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        /**
         * 当服务端完成了绑定服务的逻辑后，就会把接下来的逻辑交给客户端实现
         *
         * @param token  继承了 Binder的 ServiceRecord对象
         * @param rebind 是否是重新连接
         */
        public final void scheduleBindService(IBinder token, Intent intent,
                                              boolean rebind, int processState) {
            updateProcessState(processState, false);
            // 创建一个 BindServiceData对象，用于封装信息，方便Message携带给Handler处理
    ...
    private void handleBindService(BindServiceData data) {
        // data.token：继承了 Binder的 ServiceRecord对象
        // 根据 Binder对象获取客户端要绑定的服务
        ...
        if (s != null) {
            try {
                ...
                data.intent.prepareToEnterProcess();
                try {
                    // 如果不是重新绑定服务，第一次绑定会走到这里
                    if (!data.rebind) {
                        // 这里就会调用服务端的 onBind() 方法，获取服务端返回的Binder
                        IBinder binder = s.onBind(data.intent);
                        // 到这里，就开始回调客户端的两个回调方法了
    ...
    final void handleResumeActivity(IBinder token,
        ...
        if (r != null) {
            ...
            // 如果该Activity是第一个启动，而不是重新调用 onResume()方法重新启动
            if (r.window == null && !a.mFinished && willBeVisible) {
                // 获取当前 Activity的 PhoneWindow对象
                r.window = r.activity.getWindow();
                // 获取 DecorView，该DecorView已经在Activity的onCreate()方法的setContentView()初始化了
                View decor = r.window.getDecorView();
                decor.setVisibility(View.INVISIBLE);
                // 获取WindowManager的实现类WindowManagerImpl
                // WindowManagerImpl实现了ViewManager接口
                ViewManager wm = a.getWindowManager();
                // 为DecorView添加到Window设置一个LayoutParams
                WindowManager.LayoutParams l = r.window.getAttributes();
                // Activity和DecorView联系起来
                a.mDecor = decor;
    ...
    private void handlePauseActivity(IBinder token, boolean finished,
        ...
        if (r != null) {
            ...
            // 分发Activity的pause事件给Activity，Activity先回调给Fragment的Pause,然后才会回调Activity的Pause
            ...
            if (!dontReport) {
                try {
                    // 这里是应用进程告诉服务进程，栈顶Activity已经执行完成onPause方法
    ...
    public static ActivityThread systemMain() {
        ...
        // 实例化一个 ActivityThread的时候会初始化 ResourcesManager
        ActivityThread thread = new ActivityThread();
        // true：说明这个attach()方法是由系统调用的，对于系统调用该方法会有不同的逻辑
        thread.attach(true);
        return thread;
    }
