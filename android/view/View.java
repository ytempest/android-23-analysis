...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    public boolean performClick() {
        ...
        // 如果设置了OnClickListener，那么就调用其onClick()方法
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (onFilterTouchEventForSecurity(event)) {
            //noinspection SimplifiableIfStatement
            ListenerInfo li = mListenerInfo;
            // 判断是否设置了触摸事件监听器，如果有就把触摸事件回调给监听器，并判断监听器是否消费
            // 该触摸事件，如果消费了则不再往下传递了，即不会再调用View的 onTouchEvent()方法
     ...
     * 如果这个View不是VISIBLE同时这个View没有在运行动画，那么就不会刷新View
     */
     ...
     * 如果View是visible，那么就重绘整个View；调用该方法后会调用该View的onDraw()方法
     * {@link #onDraw(android.graphics.Canvas)} will be called at some point in
     * the future.
     * <p>
     * This must be called from a UI thread. To call from a non-UI thread, call
     * 这个方法必须要在UI线程中进行调用，如果要想在非UI线程调用，那么可以使用postInvalidate()
     * {@link #postInvalidate()}.
     */
     ...
     * 这里是invalidate()真正发生的地方。一个完全的刷新会导致绘制缓存无效，但是绘制缓存的功能可以
     * 通过设置 invalidateCache为false来进行使用（使用绘制缓存的功能的例子如：一个组件在相同的维度
     * 上具有相同的内容）
     *
     ...
     *                        用于表示这个View的绘制缓存是否无效。这个通常设置为true用于一个完全的刷
     *                        新，但是如果一个View的内容或者维度没有发生改变，那么可能会设置为false
     */
    void invalidate(boolean invalidateCache) {
        // 最后一个参数设置为true表示这是一个完全的刷新
    ...
    void invalidateInternal(int l, int t, int r, int b, boolean invalidateCache,
        ...
        // 判断是否跳过刷新，如果说该View没有运行动画，同时是GONE或者INVISIBLE，那么就会跳过刷新
        if (skipInvalidate()) {
            return;
        }
        ...
        if ((mPrivateFlags & (PFLAG_DRAWN | PFLAG_HAS_BOUNDS)) == (PFLAG_DRAWN | PFLAG_HAS_BOUNDS)
            ...
            // 添加需要重新绘制的标记位
            // PFLAG_DIRTY这个标记位表示该View是否需要进行部分重绘或者完全重绘
            mPrivateFlags |= PFLAG_DIRTY;

            // 如果需要使用绘制缓冲，那么会添加PFLAG_INVALIDATED这个标记位，同时移除
            // PFLAG_DRAWING_CACHE_VALID这个标记位
            if (invalidateCache) {
            ...
            if (p != null && ai != null && l < r && t < b) {
                ...
                // 设置该View发生改变后的矩形位置
                damage.set(l, t, r, b);
                // 交给父布局对该View的矩形位置进行刷新
                // ViewGroup的子类都没有对这个方法进行重写，那么直接看到ViewGroup中
     ...
     * 判断该View是否是透明的，如果是透明的则返回true，否则返回false
     * <p>
     ...
     * 这个方法会由 ViewGroup.drawChild()方法进行调用，用于绘制ViewGroup的子View
     */
    boolean draw(Canvas canvas, ViewGroup parent, long drawingTime) {
        ...
        // 尽可能早的设置PFLAG_DRAWN标记位以确保使用draw()方法实现动画时能成功调用invalidate()方法
        mPrivateFlags |= PFLAG_DRAWN;
        ...
        if (!drawingWithDrawingCache) {
            if (drawingWithRenderNode) {
                ...
                // Fast path for layouts with no backgrounds
                // 如果该ViewGroup没有背景，那么就直接分发绘制事件以达到快速运行
                if ((mPrivateFlags & PFLAG_SKIP_DRAW) == PFLAG_SKIP_DRAW) {
                    mPrivateFlags &= ~PFLAG_DIRTY_MASK;
                    dispatchDraw(canvas);
                } else {
                    // 如果ViewGroup有背景或者该View不是ViewGroup都会走这个方法
                    draw(canvas);
                }
            }
        } else if (cache != null) {
    ...
    public void draw(Canvas canvas) {
        ...
        mPrivateFlags = (privateFlags & ~PFLAG_DIRTY_MASK) | PFL    AG_DRAWN;
