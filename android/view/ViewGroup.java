...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (onFilterTouchEventForSecurity(ev)) {
            ...
            // intercepted为true，表示ViewGroup会拦截这个触摸事件，然后自己消费，不会分发下去
            // intercepted为false，表示ViewGroup不拦截这个触摸事件，而是将事件分发下去
            final boolean intercepted;
            // 如果是ACTION_DOWN事件 或者mFirstTouchTarget不为空 就会让本ViewGroup判断是否需要拦截触摸事件
            // 只有ViewGroup将ACTION_DOWN事件分发给子View，mFirstTouchTarget才会赋值并不为空，也就是
            // 在这之后都会调用ViewGroup的onInterceptTouchEvent()方法对每一个触摸事件判断是否拦截
            if (actionMasked == MotionEvent.ACTION_DOWN
                    || mFirstTouchTarget != null) {
                // 判断用户是否设置了不允许本ViewGroup拦截触摸事件
                final boolean disallowIntercept = (mGroupFlags & FLAG_DISALLOW_INTERCEPT) != 0;
                // 如果允许本ViewGroup拦截触摸事件
                if (!disallowIntercept) {
                    // ViewGroup的onInterceptTouchEvent()默认返回false，不拦截事件
                    // RecyclerView等会重写这个方法实现拦截
            ...
            // 如果触摸事件没有取消，同时本ViewGroup也没有拦截，那么就会准备遍历子View并分发触摸事件
                ...
                if (actionMasked == MotionEvent.ACTION_DOWN
                    ...
                    if (newTouchTarget == null && childrenCount != 0) {
                        ...
                        for (int i = childrenCount - 1; i >= 0; i--) {
                            ...
                            // dispatchTransformedTouchEvent()方法会将触摸事件分发给子View
    ...
    private TouchTarget addTouchTarget(View child, int pointerIdBits) {
        // 将View封装成一个TouchTarget
        TouchTarget target = TouchTarget.obtain(child, pointerIdBits);
        // 将这个TouchTarget加入到链的开头
        target.next = mFirstTouchTarget;
        // 为mFirstTouchTarget赋值，即指向最新的TouchTarget对象
    ...
    private boolean dispatchTransformedTouchEvent(MotionEvent event, boolean cancel,
        ...
        if (child == null) {
            // 如果是ViewGroup拦截了ACTION_DOWN事件，那么child为空，会调用父类的dispatchTouchEvent()分发事件
            ...
            // 如果child不为空则表示ViewGroup不拦截ACTION_DOWN事件
     ...
     * the view hierarchy.
     * 使用者不要调用或者重写这个方法。这个方法用于实现View的层次结构
     *
     * @param child 要进行刷新到的View
     * @param dirty 该View要刷新的矩形区域，也就是这个View所占据的矩形区域
     */
    public final void invalidateChild(View child, final Rect dirty) {
        ...
        if (attachInfo != null) {
            ...
            // through
            // 判断该View是否正在进行动画，如果正在进行动画，那么就会存在PFLAG_DRAW_ANIMATION标记位
            ...
            // 一直循环，直到获取要刷新的View的最外层View（一般是ViewRootImpl）
            do {
                // 这个是View的最外层的ViewGroup
                View view = null;
                // 如果要刷新的View的父布局是ViewGroup，那么就会满足以下条件
                // 也就是获取View的最外层的ViewGroup，而ViewRootImpl不是ViewGroup
                ...
                if (drawAnimation) {
                    if (view != null) {
                        // 为该View的最外层的ViewGroup也添加PFLAG_DRAW_ANIMATION进行动画的标志位
                ...
                // 如果这个view不为空，表示这个view就是最外层的ViewGroup
                if (view != null) {
                ...
                // 获取parent的父布局，也就是当前ViewGroup的父布局
                // 在这个do-while循环中不断遍历，直到遍历到要刷新View的最外层View，这个最外层View也就是
                // ViewRootImpl，所以继续看到这个类的的 invalidateChildInParent()方法
                /**{@link ViewRootImpl#invalidateChildInParent(int[], Rect)}*/
     ...
     * 如果该ViewGroup没有父布局，那么直接返回空；如果这个ViewGroup已经完全刷新过了 或者 要刷新的
     * View的矩形位置不在ViewGroup的边界之内，那么也返回空
     * 其他情况则返回这个ViewGroup的父布局
     */
