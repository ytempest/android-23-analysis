...
public final class ViewRootImpl implements ViewParent,
    ...
    /**
     * 这个方法会在 ActivityThread中的 handleResumeActivity()方法中将 DecorView添加到 Window时调用
     *
     * @param view  这个View就是 DecorView
     * @param attrs 将 DecorView添加到 Window所需要的 LayoutParams
     */
    ...
    /**
     * 当View使用invalidate()方法刷新View的时候，最终会调用到最外层，即ViewRootImpl的 invalidateChildInParent()方法中
     *
     * @param location 要进行刷新的View的位置
     * @param dirty    View要刷新的矩形位值
     * @return 返回null，因为ViewRootImpl已经是最顶层的View了
     */
    @Override
    public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
        ...
        // 如果要要刷新的矩形位置为null，那么就会刷新整个View树
        if (dirty == null) {
            invalidate();
            return null;

        } else if (dirty.isEmpty() && !mIsAnimating) {
            // 如果要刷新的矩形区域为空(即右边>=左边 or 顶部>=底部)，同时该View并没有在运行动画，那么
            // return，不进行重绘
            return null;
        }
    ...
    private void performTraversals() {
        ...
        /*------- 如果该View是第一次添加进来，那么就会执行mFirst为true这一块逻辑，否则不会执行 ------*/
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
            ...
            if (!mStopped || mReportNextDraw) {
                ...
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                    ...
                    // Ask host how big it wants to be
                    // 8、准备 measure View
                    // 只有该View是第一次添加进来，或者Window的大小发生改变或者重置才会进行测量
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);

                    // Implementation of weights from WindowManager.LayoutParams
                    // We just grow the dimensions as needed and re-measure if
                    // needs be
                    // 因为已经对 View测量过了宽高，所以这里可以获取 View的宽高了
                    ...
                    // 如果该View进行了Measure，那么就需要对View进行重新摆放
        ...
        // 调用View绘制树的监听器的dispatchOnPreDraw()方法，判断是否需要取消View的绘制
        // 或者View的可见性不是VISIBLE，那么也取消绘制
        ...
        if (!cancelDraw && !newSurface) {
            ...
            // 11、当View设置为空 VISIBLE后就会调用 scheduleTraversals()方法重新调度View的绘制任务
            if (viewVisibility == View.VISIBLE) {
                // Try again
                scheduleTraversals();
    ...
    private void draw(boolean fullRedrawNeeded) {
        ...
        if (!dirty.isEmpty() || mIsAnimating || accessibilityFocusDirty) {
            if (mAttachInfo.mHardwareRenderer != null && mAttachInfo.mHardwareRenderer.isEnabled()) {
                ...
                // 如果是View调用invalidate()方法刷新View，那么也会调用 drawSoftware()方法
     ...
     * 如果返回true则表示绘制成功；返回false则表示出现了一个错误
     */
    private boolean drawSoftware(Surface surface, AttachInfo attachInfo, int xoff, int yoff,
        ...
        try {
            ...
            // 从suface对象中获取canvas对象，同时锁住画布，保证canvas对象的线程安全
        ...
        try {
            ...
            try {
                ...
                // 如果子View没有重写该方法的话就会调用View的draw()方法
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        private int processPointerEvent(QueuedInputEvent q) {
            ...
            // mView就是DecorView，触摸事件最初就是由DecorView进行分发处理
            // dispatchPointerEvent()是View定义的final方法
