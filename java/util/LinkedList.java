...
public class LinkedList<E> extends AbstractSequentialList<E> implements
    ...
    // 底层维护的链表的头节点，这个节点的previous指向链表的最后一个节点，next指向头节点的下一个节点
    ...
    private static final class Link<ET> {
        ...
        // 节点的的前节点指针和后节点指针，这就表明这个LinkedList是一个双向链表
     ...
     * 创建一个空的链表，同时将头节点的前节点和后节点都指向头节点，形成双向链表
     */
    public LinkedList() {
     ...
     * <p>
     * 将元素插入到链表的指定索引位置，其中会根据这个索引位置判断是从开头遍历查找索引位置还是从结尾
     * 遍历查找索引位置
     *
    ...
    public void add(int location, E object) {
        // 判断该location是否超出链表的范围
        if (location >= 0 && location <= size) {
            Link<E> link = voidLink;
            // 根据索引配置判断从开头还是结尾开始进行遍历，并获取到该索引位置的节点
            if (location < (size / 2)) {
            ...
            // 将新节点的前节点设置为原索引位置的节点的上一个节点，将新节点的节点设置为原索引位置的节点
            Link<E> newLink = new Link<E>(object, previous, link);
            // 将原索引位置的节点的上一个节点的后节点设置为新节点
     ...
     * add()方法会默认将元素添加到链表的结尾，由于LinkedList没有对数据元素进行判断，也就是说
     * LinkedList可以存储多个null的数据
     *
     * @param object the object to add.
     * @return always true
     */
     ...
     * <p>
     * 在指定索引位置开始添加集合中的元素，这个方法整体都和addAll()方法一样，除了它会获取指定索引
     * 位置的节点（addAll()方法默认获取链表结尾的节点，然后将所有集合元素添加到这个节点后面）
     *
     * @param location   the index at which to insert.
    ...
    public boolean addAll(Collection<? extends E> collection) {
        ...
        // 判断要添加的集合是否为本类集合，如果是就用一个ArrayList存储本类的数据元素
        Collection<? extends E> elements = (collection == this) ?
                new ArrayList<E>(collection) : collection;

        // 获取头节点的前节点，也就是previous表示链表的最后一个节点
        Link<E> previous = voidLink.previous;
        for (E e : elements) {
            // 创建一个Link封装数据，并将Link的前节点指向上一个节点，也就是把逆向链连接起来
            Link<E> newLink = new Link<E>(e, previous, null);
            // 将新节点添加到链表的最后，也就是把顺向链连接起来
            previous.next = newLink;
            // 将链表的最后一个节点指向新节点
            previous = newLink;
        }
        // 将最后一个节点的后节点指向头节点，也就是保证链表能形成循环
    ...
    private boolean addFirstImpl(E object) {
        ...
        // 将数据封装成Link节点
        // 同时将头节点设置为该节点的前节点，设置原来的链表的第一个节点为这个节点的后节点
     ...
     * 清空链表，也就是将头节点的前节点和后节点都指向头节点，这样就完成了清空
     *
     * @see List#isEmpty
     * @see #size
     */
    ...
    /**
     * 进队：将元素添加到链表的结尾，与poll()方法配合使用从而可以实现队列的数据结构
     */
    ...
    /**
     * 出队：如果链表存在元素，则移除链表的第一个元素并返回，与offer()方法配合使用从而可以实现队列的数据结构
     */
    public E poll() {
